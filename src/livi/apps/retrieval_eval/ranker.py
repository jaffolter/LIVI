...
class Ranker:
    """
    Class for evaluating retrieval models on Music Cover Retrieval task.
    """
    ...
    def __init__(
        ...
        print(f"{len(embeddings)} embeddings, {len(self.queries_ids)} queries, {len(self.corpus_ids)} corpus")
    ...
    def compute_similarity_matrix(self) -> np.ndarray:
        q = np.vstack([self.embeddings[qid] for qid in self.queries_ids])
